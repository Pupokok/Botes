...
from keyboards import (
    cars_submenu_models_audi_inline_keyboard,
    kb_client,
    main_inline_keyboard,
    cars_submenu_inline_keyboard,
    mots_submenu_inline_keyboard,
    cars_submenu_models_lexus_inline_keyboard,
    cars_submenu_models_mers_inline_keyboard,
    cars_submenu_models_bmw_inline_keyboard
)


async def start(message: types.Message):
    await bot.send_message(message.from_user.id, 
                           "Good Morning, please don't click - Secret",
                           reply_markup=kb_client)

async def services_inkb_b1(message: types.Message):
    await bot.send_message(message.from_user.id, 
                           'What kind of transport do you want to buy?', 
...
async def secret_ddo(chat_id, total_message):
    for i in range(total_message):
        message = (
            ...
            "опиума что-ли скушал, архипиздрит"
            f"ты недоделанныый {i+1} out of {total_message}."
        )
        sleep(2)
...
async def lexus_lx_catalog(callback_query: types.CallbackQuery):
    await callback_query.message.answer('Welcome to the models Lexus LX,' 
                                        'Have a good look at the models')
    sleep(2.5)
    await sqlite.sql_write(callback_query.message.chat.id, [1])
    await callback_query.answer()

async def lexus_rx_catalog(callback_query: types.CallbackQuery):
    await callback_query.message.answer('Welcome to the models Lexus RX.'
                                        'Have a good look at the models')
    sleep(2.5)
    await sqlite.sql_write(callback_query.message.chat.id, [2, 3])
    await callback_query.answer()

async def audi_a6_catalog(callback_query: types.CallbackQuery):
    await callback_query.message.answer('Welcome to the models Audi A6.'
                                        'Have a good look at the models')
    sleep(2.5)
    await sqlite.sql_write(callback_query.message.chat.id, [4])
    await callback_query.answer()

async def audi_a7_catalog(callback_query: types.CallbackQuery):
    await callback_query.message.answer('Welcome to the models Audi A7.'
                                        'Have a good look at the models')
    sleep(2.5)
    await sqlite.sql_write(callback_query.message.chat.id, [5])
    await callback_query.answer()

async def mers_cls_catalog(callback_query: types.CallbackQuery):
    await callback_query.message.answer('Welcome to the models Mersedes CLS.'
                                        'Have a good look at the models')
    sleep(2.5)
    await sqlite.sql_write(callback_query.message.chat.id, [6])
    await callback_query.answer()

async def mers_w140_catalog(callback_query: types.CallbackQuery):
    await callback_query.message.answer('Welcome to the models Mersedes S-Class W140.'
                                        'Have a good look at the models')
    sleep(2.5)
    await sqlite.sql_write(callback_query.message.chat.id, [7])
    await callback_query.answer()
